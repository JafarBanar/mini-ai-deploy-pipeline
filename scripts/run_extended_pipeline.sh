...
python src/benchmark.py --backend "$BACKEND" --model artifacts/model.onnx --device "$DEVICE" --precision "$PRECISION" --out artifacts/bench.json --warmup 50 --iters 500 --telemetry-jsonl artifacts/telemetry/extended.jsonl
python src/benchmark_compare.py --fp32-onnx artifacts/model.onnx --int8-onnx artifacts/model.int8.onnx --mode core --out artifacts/bench_compare.json
python src/experiment_grid.py --onnx artifacts/model.onnx --mode core --batch-sizes 1,4,8 --warmups 20,50 --iters-list 200,500 --out artifacts/experiments.json
